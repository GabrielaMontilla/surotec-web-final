...
const Employees = () => {
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm("¿Estás seguro de que quieres desactivar este empleado?")
    )
      return;
    setDeletingId(id);
    try {
    ...
    try {
      ...
      const updatedEmployee = {
        ...
        hireDate: editFormData.hireDate
          ? editFormData.hireDate + "T00:00:00"
          : editingEmployee.hireDate,
      };
      await api.put(
        `/employees/${editingEmployee.idEmployee}`,
        updatedEmployee,
      );
    ...
    try {
      ...
      const newEmployee = {
        ...
        hireDate: createFormData.hireDate
          ? createFormData.hireDate + "T00:00:00"
          : null,
        userDto: {
          ...
          age: createFormData.age
            ? Math.floor(
                (new Date() - new Date(createFormData.age)) /
                  (1000 * 60 * 60 * 24 * 365),
              )
            : 0,
  ...
  const filteredEmployees = employees.filter(
    (e) =>
      `${e.userDto?.firstName || ""} ${e.userDto?.lastName || ""}`
        .toLowerCase()
  ...
  const inactive = employees.filter(
    (e) => e.userDto?.status === "INACTIVE",
  ).length;

  if (loading)
    return <div className="employees-loading">Cargando empleados...</div>;
          ...
          <tbody>
            {filteredEmployees.map((e) => (
              <tr key={e.idEmployee}>
                ...
                <td>
                  <span
                    className={`status-badge ${e.userDto?.status === "ACTIVE" ? "active" : "inactive"}`}
                  >
